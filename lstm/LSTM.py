...
# # 数据预处理
# categoryVariableList = ["season","holiday","workingday","weathersit"]
...
# testData = testData.replace([np.inf, -np.inf], np.nan).dropna()
...
class LSTMPredictor(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2, output_size=OUTPUT_LENGTH):
        super().__init__()
        print(input_size)
...
model = LSTMPredictor(X_train.shape[2]).to(device)
...
# print(train_losses)
# 绘制训练损失曲线
# plt.figure(figsize=(12, 6))  # 更合适的长宽比
# plt.plot(train_losses, label='Training Loss', color='blue', linewidth=2)
# plt.xlabel('Epoch')
# plt.ylabel('Loss')
# plt.title(f'LSTM Training Loss ({OUTPUT_LENGTH}_day)')
# plt.grid(True)
# plt.legend()

# # 保存并提示保存路径
# save_path = f'lstm/LSTM_{OUTPUT_LENGTH}_training_loss.png'
# plt.savefig(save_path, dpi=300)  # 提高图像分辨率
# 画图从第2个epoch开始
...
plt.title(f'LSTM Training Loss ({OUTPUT_LENGTH}_day)', fontsize=16)
...
# 保存与显示
plt.savefig(f'lstm/LSTM_{OUTPUT_LENGTH}_training_loss.png', dpi=300)
plt.show()
plt.close()
...
for i in range(NUM_EXPERIMENTS):
    ...
    input_reshaped[:, -1] = partial_X_test[0, :, 0].cpu().numpy()
    ...
    # 绘制输入序列
    # plt.plot(range(INPUT_LENGTH), input_seq, 'b-', label='Input Sequence')
    # 绘制预测值和真实值
    # plt.plot(range(INPUT_LENGTH, INPUT_LENGTH + OUTPUT_LENGTH), y_pred, 'r-', label='Prediction')
    # plt.plot(range(INPUT_LENGTH, INPUT_LENGTH + OUTPUT_LENGTH), y_test_real, 'g--', label='Ground Truth')
    ...
    plt.ylabel('Value')
    plt.title(f'LSTM Prediction({OUTPUT_LENGTH}_day)')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'lstm/LSTM_{OUTPUT_LENGTH}_prediction_{i+1}.png')
    plt.close()

# 输出最终评估结果
print(f'LSTM {OUTPUT_LENGTH}_day_ouput')
